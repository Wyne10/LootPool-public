...
class ComposeCommand<T : LootPool>(lootPoolProvider: LootPoolProvider, lootPoolType: Class<T> = LootPool::class.java as Class<T>) : SubCommand("compose") {
    override val command: CommandAPICommand = super.command
        ...
        .withArguments(poolMapArgument("pools") { lootPoolProvider.getMapOf(lootPoolType) })
        .executes(CommandExecutor { sender, args ->
            val key = args.getByClass("key", String::class.java)!!
            assertLootPoolNotExists(key, sender, lootPoolProvider.getMapOf(lootPoolType))
            val pools = parsePoolMap(args.getByClass("pools", String::class.java)!!, sender, lootPoolProvider.getMapOf(lootPoolType)) { it.toInt() }
...
fun poolMapArgument(nodeName: String, lootPoolMap: () -> Map<String, LootPool>): Argument<String> =
    GreedyStringArgument(nodeName)
        .replaceSuggestions(ArgumentSuggestions.strings { info ->
            val input = info.currentArg()
            val head = input.substringBeforeLast(' ', "")
            val last = input.substringAfterLast(' ')
            if (last.contains(':')) return@strings emptyArray()
            val prefix = if (head.isEmpty()) "" else "$head "
            val used = head.split(' ').map { it.substringBefore(':') }.toSet()
            lootPoolMap().keys
                .filter { it !in used && it.startsWith(last) }
                .map { "$prefix$it:" }
                .toTypedArray()
        })
        ...
        .associate { entry ->
            ...
            if (key.isEmpty() || value.isEmpty())
                throw CommandAPIBukkit.failWithAdventureComponent(
                    sender.placeholderComponent("error-invalid-pool-entry", "entry" replace entry).get()
                )
            assertLootPoolExists(key, sender, lootPoolMap)
            val mapped = runCatching { valueMapper(value) }.getOrElse {
                throw CommandAPIBukkit.failWithAdventureComponent(
                    sender.placeholderComponent("error-invalid-pool-entry", "entry" replace entry).get()
                )
            }
...
fun assertValidKey(key: String, sender: CommandSender) {
    if (NamespacedKey.fromString("lootpool:$key") != null) return
    throw CommandAPIBukkit.failWithAdventureComponent(
        sender.placeholderComponent("error-invalid-key", "key" replace key).get()
    )
}
